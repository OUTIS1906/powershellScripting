Function Get-AutodeskLanguagePack {
    $TempPath = "C:\Temp\Autodesk"
    if(-not(Test-Path -Path $TempPath -PathType Container)){ New-Item -Path $TempPath -ItemType Directory -Force | Out-Null }
    ...
    $VersionData = if($ProductData){ $LanguagePackData[$ProductData].Keys | Out-GridView -Title "Select Version" -OutputMode Single }
    
    $LanguageData = if($VersionData){ $LanguagePackData[$ProductData][$VersionData] | Out-GridView -Title "Select Language" -OutputMode Multiple }
    ...
    foreach($language in $LanguageData){
        ...
        try{
            Write-Host "Downloading $Message"
            $WebClient.DownloadFile($language.Value,$DestinationPath)

            Write-Host "Installing  $Message"
